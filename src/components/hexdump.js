...
export function Hexdump(data, options) {
	...
	case 2:
		...
		self.baseConvert = function (characters) {
			for (let i = 0; i < characters.length; i++) {
				let ddx = characters[i].charCodeAt(0), r = "";

				for (let bbx = 0; bbx < 8; bbx++) {
					r = (ddx % 2) + r;
	...
	const regex = new RegExp('.{1,' + this.options.width + '}', 'g');
	...
	for (let i = 0; i < self.data.length; i++) {
		const tempData = self.process(self.data[i]);
...
Hexdump.prototype.dump = function () {
	...
	self.output = '';
	for (let i = 0; i < self.hexdump.length; i++) {

		if (self.options.lineNumber) {
			let tempLineNumberStyle = '';
			tempLineNumberStyle += self.options.style.lineNumberLeft;

			const currentLineCount = (i * self.options.width); //.toString(16);
			const temLineCount = 8 - currentLineCount.toString().length;
			for (let l = 0; l < temLineCount; l++) {
		...
		for (let x = 0; x < self.hexdump[i].data.length; x++) {
	...
	const hexdump_container = document.getElementById(this.options.container);
...
Hexdump.prototype.splitNulls = function (code) {
	...
	if (code && code.length > 2) {
		for (let cc = 0; cc < code.length; cc++) {
			const tempi = cc + 1;
...
Hexdump.prototype.process = function (data) {
	...
	if (hexArray.length < splitHexWidth) {
		const amount = (splitHexWidth - hexArray.length);
	...
	if (stringArray.length < self.options.width) {
		const stringAmount = self.options.width - stringArray.length;
...
Hexdump.prototype.setNullPadding = function (padding) {
	...
	const hexNull = self.options.style.hexNull[0];
...
Hexdump.prototype.addPadding = function (ch, padding) {
	const self = this, length = ch.toString().length;
	let pad = '';
...
Hexdump.prototype.checkForNonPrintable = function (character) {
	const self = this;
	const c = character.charCodeAt(0).toString(16);
